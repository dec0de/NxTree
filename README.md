...
NxTree currently provides a runnable database-backed app version with a current MeeTree-style UI shell. You can enable the app, create database-backed trees, open and switch trees from the File menu, import `.mtre` files from Nextcloud Files or browser upload, export the selected branch to Nextcloud Files or browser download, view stored nodes with tree connector lines and branch expand/collapse, edit node titles and MeeTree-compatible Markdown content, change tree structure with add/delete/sort/drag-drop operations, and receive polling-based remote updates.
